...
public class MainPageObject {
    ...
    {
        int element_location_by_y = this.waitForElementPresent(locator, "Cannot find element by locator", 1).getLocation().getY(); //получить элемент по оси y
        int screen_size_by_y = driver.manage().window().getSize().getHeight(); //длина всего экрана
        return element_location_by_y < screen_size_by_y; //пока переменная больше экрана по высоте возвращается false, когда элемент найден - true
    }
